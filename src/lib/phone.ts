...
/** ITU country calling codes — any zone can sign in; user picks the country. */
...
/** Build E.164 from a country dial code + national number, or a pasted +international number. */
export function toE164(iso: string, national: string): string | null {
  ...
  const country = DIAL_COUNTRIES.find((c) => c.iso === iso);
  if (!country) return null;
  let local = digitsOnly(candidate);
  if (!local) return null;
  if (local.startsWith(country.dial)) {
...
export function defaultCountryIso(hint?: string | null): string {
  const iso = (hint || "").trim().toUpperCase();
  if (iso && DIAL_COUNTRIES.some((c) => c.iso === iso)) return iso;
  return "SG";
}
...
export function authMethodLabel(authType?: string | null): string {
  if (authType === "email") return "Email & password";
  if (authType === "google") return "Google";
  if (authType === "phone" || authType === "sms") return "Phone (SMS)";
  if (authType === "wechat") return "WeChat";
